...
procedure xuli;
var last,i,j,j1,k,p:longint; ok:boolean;
begin
 qsort(1,n);    last:=1;
 a[n+1]:=-1;
 for i:=1 to n do
 ...
 f[0,0]:=0;
 if dem>=10 then p:=9 else p:=dem-1;
 for i:=0 to p do
  ...
  if f[i,j]<>-1 then
   begin
    ...
    ok:=true;
    for k:=0 to i-1 do
     if getbit(k,j)=1 then
      if tm(k+1,i+1) then ok:=false;
    if (ok=false) then continue;
    f[i+1,j1]:=max(f[i+1,j1],f[i,j]+sl[i+1])
   end;
 //duyet(1,0);
...
begin
 assign(input,'seq198.inp');reset(input);
 assign(output,'seq198.out');rewrite(output);
 nhap;
 xuli;
