...
begin
 ...
 for i:=1 to cr do
  begin
   for j:=1 to cd do
    if abs(a[u+i-1,j]-b[i,n-(cd-j)])<=k then h[j]:=h[j]+1
    else h[j]:=0;
   timl; timr;
   for j:=1 to cd do res:=max(Res,h[j]*(r[j]-l[j]+1));
  end;
...
begin
 ...
 for i:=1 to cr do
  begin
   for j:=1 to cd do
    if abs(a[i,j]-b[m-(cr-i),n-(cd-j)])<=k then h[j]:=h[j]+1
    else h[j]:=0;
   timl; timr;
   for j:=1 to cd do res:=max(Res,h[j]*(r[j]-l[j]+1));
  end;
...
begin
 ...
 for i:=1 to cr do
  begin
   for j:=1 to cd do
    if abs(a[i,v+j-1]-b[m-(cr-i),j])<=k then h[j]:=h[j]+1
    else h[j]:=0;
   timl; timr;
   for j:=1 to cd do res:=max(Res,h[j]*(r[j]-l[j]+1));
  end;
